...
4. Reserve the `snowbros` name on npm (`npm publish` from `npm/`) and on
   crates.io (`cargo publish -p snowbros_core …` chain, or publish on the
   first release).
...
- uploads `snowbros-installer.sh` / `snowbros-installer.ps1`,
...
# Publish to crates.io so `cargo install snowbros --locked` works.
cargo publish -p snowbros_core   # then dependents in dependency order,
cargo publish -p snowbros        # ending with the CLI package
```
...
# Homebrew
brew install snowbros/tap/snowbros && sb --version
